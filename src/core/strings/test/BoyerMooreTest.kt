...
class BoyerMooreTest {
    ...
    @Test fun simpleCase(): Unit {
        ...
        var boyerMoore = BoyerMoore("a")
        assertEquals(-1, boyerMoore.search(""))
        assertEquals(0, boyerMoore.search("a"))
        assertEquals(-1, boyerMoore.search("b"))
        assertEquals(2, boyerMoore.search("bba"))
        assertEquals(1, boyerMoore.search("baba"))

        boyerMoore = BoyerMoore("abba")
        assertEquals(-1, boyerMoore.search("a"))
        assertEquals(-1, boyerMoore.search("abab"))
        assertEquals(0, boyerMoore.search("abba"))
        assertEquals(1, boyerMoore.search("aabba"))
        assertEquals(-1, boyerMoore.search("acbbaa"))
        return
    }
        ...
        for (i in 1..testsNumber) {
            ...
            (1..testChecksNumber).forEach {
                val text = randomString(
                        ...
                        random)
                assertEquals(text.indexOf(pattern), boyerMoore.search(text))
            }
        }
    }
    ...
    @Test fun extensionsTest(): Unit {
        val test: String = "ab"
        with(BoyerMoore) {
            assertEquals(1, test.BoyerMooreSearchIn("bababa"))
        }
    }
}